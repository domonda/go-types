...
func Test_Normalize(t *testing.T) {
	for input, expected := range dateTable {
		...
		if err != nil {
			t.Errorf("Normalize(%#v): %s", input, err.Error())
			continue
		}
		if normalized != expected {
			t.Errorf("Normalize(%#v): %#v != %#v", input, normalized, expected)
		}
	}
	...
	for _, invalidDate := range invalidDates {
		...
		if err == nil {
			t.Errorf("Should NOT be valid Normalize(%#v): %#v", invalidDate, normalized)
		}
	}
}
