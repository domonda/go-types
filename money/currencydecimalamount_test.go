...
func TestCurrencyDecimalAmount_String(t *testing.T) {
	ca := CurrencyDecimalAmountEUR(DecimalAmountFromCoefficient(123456, 2))
	assert.Equal(t, "EUR 1234.56", ca.String())

	// The amount's own scale is preserved, unlike CurrencyAmount which forces 2.
	assert.Equal(t, "EUR 1234.5678", CurrencyDecimalAmountEUR(DecimalAmountFromCoefficient(12345678, 4)).String())
	assert.Equal(t, "JPY 1000", CurrencyDecimalAmountJPY(DecimalAmountFromCoefficient(1000, 0)).String())

	// Empty currency omits the code.
	assert.Equal(t, "1234.56", NewCurrencyDecimalAmount("", DecimalAmountFromCoefficient(123456, 2)).String())
}

func TestCurrencyDecimalAmount_constructors(t *testing.T) {
	amount := DecimalAmountFromCoefficient(100, 2)
...
func TestCurrencyDecimalAmount_Format(t *testing.T) {
	ca := CurrencyDecimalAmountEUR(DecimalAmountFromCoefficient(123456789, 2)) // 1234567.89
...
func TestCurrencyDecimalAmount_GoString(t *testing.T) {
	ca := CurrencyDecimalAmountEUR(DecimalAmountFromCoefficient(123456, 2))
	assert.Equal(t, "{Currency: \"EUR\", Amount: money.DecimalAmountFromCoefficient(123456, 2)}", ca.GoString())
	assert.Equal(t, "{Currency: \"EUR\", Amount: money.DecimalAmountFromCoefficient(123456, 2)}", fmt.Sprintf("%#v", ca))
}
...
func TestCurrencyDecimalAmount_JSON(t *testing.T) {
	ca := CurrencyDecimalAmountEUR(DecimalAmountFromCoefficient(99999, 2))
